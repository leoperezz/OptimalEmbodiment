...
from optimal_embodiment.robot.utils import (body_names_from_model,
                                            resolve_frame_name,
                                            resolve_head_body)
...
def _resolve_body_name(model: mujoco.MjModel, template_frame: str) -> Optional[str]:
    ...
    if template_frame == "head":
        return resolve_head_body(body_names)
    return resolve_frame_name(template_frame, body_names)
...
def evaluate_robot(
    ...
    if fps not in human_frames_cache:
        human_frames, _, _ = load_human_motion_frames(
            npz_path=motion_npz,
